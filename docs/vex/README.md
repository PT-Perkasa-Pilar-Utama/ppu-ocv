...
vulnerabilities that scanners flag in ppu-ocv or its dependencies but that do
**not** affect users, because the vulnerable code path is never reached.
...
```json
{
  ...
  "@id": "https://openvex.dev/docs/ppu-ocv/CVE-YYYY-NNNNN",
  ...
  "statements": [
    {
      ...
      "products": [{ "@id": "pkg:npm/ppu-ocv@X.Y.Z" }],
      "status": "not_affected",
      "justification": "vulnerable_code_not_in_execute_path",
      "impact_statement": "Describe why the vulnerable function is never called from ppu-ocv."
    }
  ]
}
```
